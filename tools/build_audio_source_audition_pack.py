...
def verify_output_hashes(out: Path, items: Iterable[dict[str, Any]]) -> None:
    for item in items:
        ...
        if actual != item['sha']:
            raise PackError(f"output hash mismatch for {item['reviewPath']}: expected={item['sha']} actual={actual}")
...
def build_pack(repo_root: Path, artifact_paths: list[Path], out_dir: Path, *, require_pinned_artifact_wrapper: bool = False, source_sha: str | None = None) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix='oen-audition-build-') as td:
        ...
        all_items = [*main_items, *ext_items, *field_items]
        verify_output_hashes(staging, all_items)
        data = {
            'main': main_items,
            'extension': ext_items,
            'field': field_items,
            ...
            'extensionTemplate': ext_template,
            'fieldTemplate': field_template,
        }
        html_template = paths['html'].read_text(encoding='utf-8')
        marker = '__AUDITION_DATA_JSON__'
        if html_template.count(marker) != 1:
            raise PackError(f'HTML template must contain exactly one {marker} marker')
        atomic_write(staging / 'review.html', html_template.replace(marker, json.dumps(data, ensure_ascii=False, separators=(',', ':'))).encode())
        for filename, payload in (
            ('main_review.template.json', main_template),
            ('extension_review.template.json', ext_template),
            ('field_review.template.json', field_template),
        ):
            atomic_write(staging / filename, (json.dumps(payload, indent=2, ensure_ascii=False) + '\n').encode())
        manifest = {
            'version': 2,
            'status': 'audition-pack-unreviewed',
            'createdAtUtc': datetime.now(timezone.utc).isoformat(),
            'sourceMainSha': source_sha or source_main_sha(repo_root),
            'artifactVerification': [
                {
                    'file': a.path.name,
                    'sha256': a.sha256,
                    'matchesCommittedWrapperDigest': a.known_label is not None,
                    'committedWrapperLabel': a.known_label,
                    'bytes': a.path.stat().st_size,
                }
                for a in artifacts
            ],
            'counts': {'main': len(main_items), 'extension': len(ext_items), 'field': len(field_items), 'total': len(all_items)},
            'allAudioHashesVerified': True,
            'reviewPrefilled': False,
            'provenance': [*main_prov, *ext_prov, *field_prov],
            'items': all_items,
            'rule': 'Source-byte SHA verification and human review evidence do not imply source/master/runtime/release approval.',
        }
...
def main_cli() -> int:
    ...
    parser.add_argument('--require-pinned-artifact-wrapper', action='store_true')
    parser.add_argument('--source-main-sha')
    args = parser.parse_args()
    try:
        manifest = build_pack(
            args.repo_root.resolve(),
            [p.resolve() for p in args.artifact],
            args.out_dir.resolve(),
            require_pinned_artifact_wrapper=args.require_pinned_artifact_wrapper,
            source_sha=args.source_main_sha,
        )
        if args.zip_output:
            ...
            with zipfile.ZipFile(zip_path, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=6) as archive:
                for path in sorted(args.out_dir.resolve().rglob('*')):
                    if path.is_file():
                        archive.write(path, path.relative_to(args.out_dir.resolve()).as_posix())
        c = manifest['counts']
        print(f"OK audition pack: {c['total']} sources ({c['main']} main, {c['extension']} extension, {c['field']} field); all source hashes verified")
        return 0
