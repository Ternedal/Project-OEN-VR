#!/usr/bin/env python3
"""Validate the B1 environment production mesh and texture source pack."""
...
PACK = ROOT / "source_art" / "environment" / "b1" / "production"
...
EXPECTED = {
    "ENV_JUNGLE_PATH_001": {
        ...
        "minimum_parts": 50,
        "bounds": ((5.5, 2.8, 10.0), (7.5, 4.0, 12.0)),
        "parts": {"PrimaryRoute_1", "VisibleReconnectDetour_1", "CampReturnAnchor_Left", "RidgeExitMarker_Header", "ReadableResourceMass_1"},
    },
    "ENV_RAVINE_001": {
        "minimum_vertices": 180,
        "minimum_parts": 20,
        "bounds": ((6.5, 1.8, 4.0), (9.0, 3.0, 6.0)),
        "parts": {"BelayerSafeStance", "RecoveryObjectiveLedge", "BoundedProgressionPoint_1", "FailForwardReturn_1"},
    },
    "ENV_RIDGE_001": {
        "minimum_vertices": 180,
        "minimum_parts": 20,
        "bounds": ((6.5, 2.0, 5.0), (9.0, 3.5, 7.0)),
        "parts": {"SafeOverlookPlatform", "ObviousReturnRoute", "DirectionalWindPanel", "SignalDirectionCrossbar", "ComfortEdgeBarrier_1"},
    },
    "PRP_RAVINE_ANCHOR_001": {
        "minimum_vertices": 120,
        "minimum_parts": 4,
        "bounds": ((0.70, 0.60, 0.50), (1.20, 1.00, 0.90)),
        "parts": {"MountingRock", "HighContrastAnchorRing", "ReadableRopeExitLeft", "ReadableRopeExitRight"},
    },
    "PRP_RAVINE_GUIDE_MARKERS_001": {
        "minimum_vertices": 100,
        "minimum_parts": 6,
        "bounds": ((1.0, 0.06, 0.30), (1.5, 0.16, 0.55)),
        "parts": {"Marker_1_Circle", "Marker_2_Square", "Marker_3_Diamond"},
    },
}

EXPECTED_TEXTURES = {
    "MAT_B1_ENVIRONMENT_ATLAS_SOURCE_001.png": (1254, 1254),
    "MAT_JUNGLE_FOLIAGE_001.png": (627, 627),
    "MAT_RAVINE_ROCK_001.png": (627, 627),
    "MAT_RIDGE_GROUND_001.png": (627, 627),
    "MAT_ROUTE_MARKER_001.png": (627, 627),
}
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(
        "B1 environment sources OK: 5 OBJ/MTL assets and 5 PNG sources have "
        "valid bounds, UVs, semantic parts, links and dimensions."
    )
    return 0
